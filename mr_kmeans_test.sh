...
LOG_FILE="mr_kmeans_summary.log"
...
echo "" > $LOG_FILE
echo "=======================================================================" >> $LOG_FILE
echo "MR + KMeans Batch Run Started at: $(date)" >> $LOG_FILE
printf "%-30s | %-12s | %-8s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "Model" "Order" "W-PPL" "C4-PPL" "PIQA" "ARC-C" "WINO" "BOOLQ" "HELLA" >> $LOG_FILE
echo "-----------------------------------------------------------------------" >> $LOG_FILE
...
for MODEL in "${MODELS[@]}"; do
    for ORDER in "${ORDERS[@]}"; do
        echo ""
        echo "----------------------------------------------------------"
        echo "Processing: $MODEL | Transform Order: $ORDER"
        ...
        # 运行并静默非核心输出
        tmp_out="tmp_mr_eval.out"
        echo "Launch Command: $CMD" >> $LOG_FILE
        $CMD > $tmp_out 2>&1

        # 如果运行失败，记录错误
        if [ $? -ne 0 ]; then
            echo "$MODEL | $ORDER | FAILED (Check tmp_mr_eval.out)" >> $LOG_FILE
        ...
        # 打印到屏幕
        echo "Result [$ORDER]: Wiki-PPL: $WIKI_PPL, PIQA: $PIQA"
        
        # 格式化写入日志
        printf "%-30s | %-12s | %-8s | %-8s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "$MODEL" "$ORDER" "$WIKI_PPL" "$C4_PPL" "$PIQA" "$ARC" "$WINO" "$BOOLQ" "$HELLA" >> $LOG_FILE
