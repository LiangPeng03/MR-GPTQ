...
printf "%-30s | %-6s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "Model" "Lambda" "W-PPL" "C4-PPL" "PIQA" "ARC-C" "WINO" >> $LOG_FILE
echo "----------------------------------------------------------" >> $LOG_FILE
...
for MODEL in "${MODELS[@]}"; do
    for LAMBDA_VAL in "${LAMBDAS[@]}"; do
        echo ""
        echo "----------------------------------------------------------"
        echo "Processing: $MODEL | Stagger Lambda: $LAMBDA_VAL"
        ...
        # 运行并静默非核心输出
        tmp_out="tmp_eval_${LAMBDA_VAL}.out"
        echo "Launch Command: $CMD" >> $LOG_FILE
        $CMD > $tmp_out 2>&1

        # 如果运行失败，记录错误
        if [ $? -ne 0 ]; then
            echo "$MODEL | Lambda: $LAMBDA_VAL | FAILED (Check $tmp_out)" >> $LOG_FILE
        ...
        # 打印到屏幕
        echo "Result ($LAMBDA_VAL): Wiki-PPL: $WIKI_PPL, PIQA: $PIQA"
        
        # 格式化写入日志
        printf "%-30s | %-6s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "$MODEL" "$LAMBDA_VAL" "$WIKI_PPL" "$C4_PPL" "$PIQA" "$ARC" "$WINO" >> $LOG_FILE
